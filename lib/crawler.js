...
var Crawler = function(initialURL) {
    ...
    this.discoverRegex = [
        ...
        function(string) {
            var match = string.match(/<meta(?:\s[^>]*)?\shttp-equiv\s*=\s*["']?refresh["'\s](?:[^>]*\s)?content\s*=\s*["']?[^"'>]*url=([^"'>]*)["']?[^>]*>/i);
            return Array.isArray(match) ? [match[1]] : undefined;
        },
        function(string) {
            var match = string.match(/<meta(?:\s[^>]*)?\scontent\s*=\s*["']?[^"'>]*url=([^"'>]*)["'\s](?:[^>]*\s)?http-equiv\s*=\s*["']?refresh["']?[^>]*>/i);
            return Array.isArray(match) ? [match[1]] : undefined;
        }
    ];
...
Crawler.prototype.discoverResources = function(resourceText) {
    ...
    if (crawler.respectRobotsTxt && /<meta(?:\s[^>]*)?\sname\s*=\s*["']?robots["']?[^>]*>/i.test(resourceText)) {
        var robotsValue = /<meta(?:\s[^>]*)?\svalue\s*=\s*["']?([\w\s,]+)["']?[^>]*>/i.exec(resourceText);

        if (Array.isArray(robotsValue)){
            var robotsValues = robotsValue[1].toLowerCase().replace(/\s+/g,'').split(',');

            if(robotsValues.indexOf('nofollow') !== -1 || robotsValues.indexOf('none') !== -1){
                return [];
            }
        }
    }
...
Crawler.prototype.decodeBuffer = function(buffer, contentTypeHeader) {
    ...
    var embeddedEncoding = /<meta(?:\s[^>]*)?\scharset\s*=\s*["']?([\w\-]*)/i.exec(buffer.toString(undefined, 0, 512)) || [],
